...
exprfile <- args[2]
globalcov <- args[3]
geno <- args[4] # path to 117AX-filtered VCF for this chromosome
localcov <- args[5] # path to local ancestry covariate file for this chromosome
out <- args[6] # output file

keep <- "/share/hennlab/projects/TANDEM_SA/TB_Healthy_IDs.txt" # from metadata folder in this repository

system("echo 'Loading files'")
geno <- fread(cmd=sprintf("cat %s", geno), sep='\t', header=TRUE)
header <- read.table(exprfile, header = TRUE, nrow =1)
expr <- fread(cmd=sprintf("cat %s", exprfile), sep='\t', header=TRUE)
setnames(expr, colnames(header))
globalcov <- fread(globalcov, sep='\t', header=TRUE)
localcov <- fread(cmd=sprintf("cat %s", localcov), sep='\t', header=TRUE)
keep <- fread(keep, sep='\t', header=FALSE)

rows <- paste0("InferredCov", 2:110)
...
extract_id <- function(x){
   return(as.integer(unname(unlist(strsplit(x, ':')))[2]))
}

geno[,POS := sapply(VARIANT_ID,extract_id)]

minstart <- min(as.numeric(expr[,start]), na.rm = TRUE ) - 1000000
maxstart <- max(as.numeric(expr[,start]), na.rm = TRUE ) + 1000000
...
geno <- as.matrix(geno, stringsAsFactors=FALSE, check.names=FALSE)
info_col <- geno[,1:9]
    ...
    out = tryCatch(
        {
            ...
            lm(expression ~ . - AFR - NAMA - EUR - SEA - EAS - SUBJID , data=df)
...
trylmlocal <- function(df) {
    out = tryCatch(
        {
            lm(expression ~ . - PC1 - PC2 - PC3 - PC4 - NAMA - EUR - SEA - EAS - SUBJID, data=df)
        },
        error=function(cond) {
            # message(cond)
            return(NA)
...
for(j in 1:nrow(geno)){
    ...
    # make data table for local covariates
        ...
        local <- local[,.(SUBJID,AFR,NAMA,EUR,SEA,EAS)]
        strTmp = c('AFR', 'NAMA', 'EUR', 'SEA', 'EAS') # Name of columns to be converted to numeric
